...
type Category = {
  id: string;
  label: string; // supports \n for neat 2-line labels
  href?: string;
  icon: React.ComponentType<React.SVGProps<SVGSVGElement>>;
};

const CATEGORIES: Category[] = [
  { id: "nightclubs", label: "Nightclubs", href: "/categories/nightclubs", icon: Theater },
  { id: "lounges-bars", label: "Lounges &\nBars", href: "/categories/lounges-bars", icon: Martini },
  { id: "beach-parties", label: "Beach\nParties", href: "/categories/beach-parties", icon: Waves },
  { id: "house-parties", label: "House\nParties", href: "/categories/house-parties", icon: Home },
  { id: "live-music-concerts", label: "Live Music\n& Concerts", href: "/categories/live-music-concerts", icon: Music2 },
  { id: "pool-parties", label: "Pool\nParties", href: "/categories/pool-parties", icon: Droplets },
  { id: "karaoke-nights", label: "Karaoke\nNights", href: "/categories/karaoke-nights", icon: Mic2 },
  { id: "after-parties", label: "After-\nParties", href: "/categories/after-parties", icon: PartyPopper },
  { id: "boat-parties", label: "Boat\nParties", href: "/categories/boat-parties", icon: Ship },
  { id: "game-nights", label: "Game\nNights", href: "/categories/game-nights", icon: Gamepad2 },
];
...
function CategoryCard({
      ...
      <div className="flex flex-col items-center justify-center gap-3 p-5">
        ...
        <div
          className={cn(
            ...
            // Bold + professional, scales nicely
            "font-semibold",
  ...
  // Button mode (if onSelect is provided)
  return (
    <button
      type="button"
...
export function CategoriesSection({
  ...
  onSelect,
  className,
  mobileInitialCount = 6, // 2 rows x 3 columns on mobile
}: {
  categories?: Category[];
  activeId?: string;
  onSelect?: (cat: Category) => void;
  ...
  const mobileVisible = expanded ? categories : categories.slice(0, mobileInitialCount);

  return (
    <section className={cn("w-full", className)} aria-label="Categories">
      {/* MOBILE: grid + View more */}
