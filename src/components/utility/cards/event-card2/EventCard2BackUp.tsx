...
export default function EventCard2({ event, cardTag, clickHandler }: EventCardProps) {
  const [isFavorited, setIsFavorited] = useState(false);

  const formatPrice = (price?: number, currency?: string) => {
    // show Free only when price is 0 or missing
  ...
  return (
    <Card className="group w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm hover:shadow-lg transition-shadow">
      ...
      <CardContent className="p-4 sm:p-5">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex items-start gap-2 text-gray-800">
            ...
            <p className="text-[13px] sm:text-[14px] font-semibold leading-snug">
              {event?.date ? `${event.date}${event?.time ? `, ${event.time}` : ""}` : "Date/Time TBA"}
            </p>
          </div>

          {/* Price + Attending
              Desktop/tablet: side-by-side
              Mobile: stacks under date/time */}
        ...
        {/* Optional: Host/brand (kept from your old layout, but cleaner) */}
        {event?.brand?.name ? (
          <div className="mt-5 pt-4 border-t border-dashed border-gray-200 flex items-center gap-3">
            <div className="h-10 w-10 rounded-full bg-gray-900 text-white flex items-center justify-center font-bold shrink-0 overflow-hidden">
              {event?.brand?.logo ? (
                <Image
