...
const SideBar = () => {
  ...
  const { logout, currentUser } = useAppData();
  ...
  return (
    <div className="sign-up-cont w-[6.6rem] h-full [display:none] lg:flex flex-col items-center bg-[#202222] border-x-2 border-[#343434]">
      ...
      <Link href="/create-event?page=1" className="group">
        <div
          className={`flex items-center justify-center shadow-[0_0_20px_rgba(0,0,0,0.5)] group-hover:scale-105 group-hover:bg-[#f5f5f5] transition rounded-full mt-[2.5rem] p-2 2xl:p-2.5 cursor-pointer ${
            isCreateEventActive ? "bg-[#f5f5f5]" : "bg-[#F800E9]"
          }`}
        >
          <Icon
            ...
            width="24"
            height="24"
            className={`h-4 w-4 2xl:w-6 2xl:h-6 group-hover:text-zinc-900 ${
              isCreateEventActive ? "text-zinc-900" : "text-white"
            }`}
          />
        </div>
      </Link>

      <div className="mt-[4.54rem] flex flex-col items-center gap-3">
        <ToggleSwitch />
        {currentUser ? (
          <button
            type="button"
            onClick={logout}
            className="text-[10px] 2xl:text-xs text-[#F5F5F5] border border-[#393939] px-3 py-1 rounded-full hover:bg-[#393939]"
          >
            Log out
          </button>
        ) : null}
