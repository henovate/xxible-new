...
const eventInformationLayout = ({ children, event, isRsvped, onToggleRsvp }: EventInformationLayoutProps) => {
        ...
        <div className="flex items-center gap-6">
          {onToggleRsvp ? (
            <button
              ...
              className={`px-6 py-2 rounded-lg font-[500] text-base ${
                isRsvped ? "bg-[#F800E9] text-white" : "bg-[#f5f5f5] text-[#898989]"
              }`}
            >
              {isRsvped ? "Going" : "RSVP"}
            </button>
        ...
        <div className='hidden xl:block'>
          <div className='bg-[#232323] rounded-xl p-6 w-full shadow-[0_0_20px_rgba(0,0,0,0.5)]'>
            <div className='flex items-center justify-between'>
              <p className="w-[13.2rem] font-[600] text-xl leading-6">{event?.hostName ?? "Quilox"} - {event?.hostDescription ?? "Explore nightlife in Lagos"}</p>
          ...
          <div className='mt-10 px-5 pt-5 pb-10 rounded-t-2xl bg-[#232323] shadow-[0_0_20px_rgba(0,0,0,0.5)]'>
            <div className='flex gap-4'>
              ...
              <div className='text-[#E9E9E9]'>
                <p className='font-[500] leading-none text-sm'>{event?.date ?? "Saturday, June 28, 2025"}</p>
                <p className='font-[500] leading-none text-sm mt-1'>{event?.time ?? "06:00 PM WAT"}</p>
            ...
            <div className='flex gap-4 mt-9'>
              ...
              <div className='text-[#E9E9E9]'>
                <p className='font-[500] leading-none text-sm'>{event?.hostName ?? "Club Quilox"}</p>
                <p className='font-[500] leading-4 text-sm mt-1 w-[14rem] lg:w-[13rem]'>{event?.venue ?? "873 Ozumba Mbadiwe Ave, Victoria Island, Lagos 106104, Lagos"}</p>
          ...
          <div className='mt-[4.8rem]'>
            <p className="text-2xl leading-none font-[600] text-[#f5f5f5]">More Events</p>
