...
interface NoItemCardProps {
	iconName: string;
	cardDescription: React.ReactNode;
	iconClasses?: string; 
}

const NoItemCard = ({iconName, cardDescription, iconClasses}:NoItemCardProps) => {
  return (
	<div className='border border-[#E9E9E9] bg-[url("/assets/img/bg8.png")] bg-center bg-no-repeat bg-cover pb-9 px-8 lg:px-28 w-fit min-w-0 rounded-3xl mt-5'>
		<div className='mt-16 flex flex-col items-center justify-center text-center'>
			...
			<p className='font-[400] text-xs md:text-sm leading-none mt-3 text-[#A0A0A0] flex items-center gap-1 whitespace-nowrap'>{cardDescription}</p>
		</div>

		<div className="flex justify-center items-center mt-8">
			<Icon icon={iconName} width="24" height="24" className={`h-32 w-32 sm:h-48 sm:w-48 ${iconClasses}`} />
		</div>
