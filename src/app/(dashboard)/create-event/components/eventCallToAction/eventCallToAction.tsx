...
const EventCallToAction = ({children, heading, description, setSwitch, switchState, bgColor="bg-gradient-to-r from-[#5500cd38] to-[#f800e81e]"}:EventCallToActionProps) => {

	const handleToggleSwitch = () => {
		if (setSwitch) setSwitch(prev => !prev);	
	}
	...
	<div className={`border border-[#343434] p-3 xl:p-6 rounded-[0.75rem] ${bgColor}`}>
		<div className='text-[#f5f5f5] flex justify-between items-center'>
			<div className='w-[80%] md:w-full'>
				...
				<p className='font-[400] text-xs leading-3 sm:text-sm xl:text-[1.15rem] sm:leading-[0.9rem] xl:leading-none mt-2 xl:mt-3'>{description}</p>
			</div>
